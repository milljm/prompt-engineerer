...
import { applySyntax, readSyntaxPref } from "@/lib/md/syntax";
...
export function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    const onChange = () => applySyntax(readSyntaxPref(), applyTheme("system"));
  ...
  function choose(next: ThemePref) {
    ...
    applySyntax(readSyntaxPref(), applyTheme(next));
  }
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <Button
          ...
          title={`Theme: ${pref}`}
        >
          <Lightbulb className={cn("size-4", resolved === "light" && "fill-foreground/20")} />
        </Button>
          ...
          {OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                ...
                {active ? <Check className="size-3.5 text-foreground" /> : <span className="size-3.5" />}
              </DropdownMenu.Item>
            );
          })}
        </DropdownMenu.Content>
